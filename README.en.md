...
- **Roads inside prefabs are read too**: a bridge is very often a prefab, whose objects live in their own `.prefab.json` rather than in `items.level.json`, written as one document instead of a line per object. When line-by-line parsing finds nothing, the whole document is walked recursively for anything whose `class` is `DecalRoad` or `MeshRoad`.
...
- `?stats=1` reports a `files` breakdown of what was actually scanned: `items`, `prefabJSON`, and `prefabObjects` — the last being old-style Torque text `.prefab` files, which this **cannot** read, so if a level's bridges live in those, that count is the answer.
