...
- **The cache carries a format version**: keying it on the archive's size and timestamp alone is not enough — the archive has not changed, but the extractor has, and a stale cache silently defeats new parsing (which is exactly how MeshRoad bridges went on missing after support for them was added).
