...
- **The surface is drawn as one shape**: a trapezoid per segment plus **a disc at every node**, all accumulated into one path and filled once — which is how BeamNG's own navigation app draws it, a trapezoid per navgraph link with a circle at each end. Capping only the two ends of each road is not enough: bends and the joints between roads both split open. The discs must wind the **same way** as the trapezoids, or nonzero filling cancels them against each other and punches a row of notches down the middle of the road.
- **Where the level has no road geometry at all, the driven lap stands in**: some bridges — hirochi_raceway's among them — appear in no items file, no prefab JSON and no Torque prefab, because the deck is a static mesh, and no parser can recover what is not there. The car drove over it, though, so the lap is the evidence. Stretches of the reference lap with no road beneath them are filled with a corridor of its own path at the width the surrounding roads use. It only fires where data is genuinely missing: a complete network fills nothing, and removing one road by hand produces exactly one filled stretch.
- **Road ends are welded**: a short gap (a road-width plus 4 m) closes on proximity; a longer one, out to 60 m, closes only when the two roads **point at each other**. A bridge deck is its own object and the network either side can stop tens of metres short, but it stops in line with itself, which a crossing road never does. Both tests carry a height limit (`2.5 + gap × 0.15` m), so an overpass is never welded to the road beneath it.
- **Roads inside prefabs are read, in both formats**: a bridge is very often a prefab. The newer form is `.prefab.json`, one document rather than a line per object, so parsing falls back to walking the whole document for anything whose `class` is `DecalRoad` or `MeshRoad`. The older form is **TorqueScript `.prefab`** — hirochi_raceway carries 8 JSON prefabs against **56 Torque ones**, and that is where its bridges are. Those look like `new DecalRoad(name) { Material = "…"; drivability = "1"; Node = "x y z width"; };`; field names change case between versions and objects nest, so the parser matches case-insensitively and tracks brace depth rather than assuming a shape.
- **The cache is keyed on the format AND the build**: the archive's size and timestamp are not enough, because the archive has not changed — the extractor has, and a stale cache silently defeats new parsing. That caught MeshRoad bridges and then prefab roads, both times because the manual version bump was forgotten; keying on the build makes an upgrade always re-read.
- `?stats=1` reports a `files` breakdown of what was actually scanned: `items`, `prefabJSON`, `prefabTorque`, and `prefabRoads` — how many roads came out of prefabs.
...
**Hovering the map changes nothing; only a click sets the cursor.** The cursor drives the charts, the gauges and the readout, and the map’s own corner holds the RD / N / 3D buttons — so every trip across the map to reach them handed the position being read to whatever line the pointer passed, which is what reads as the dot chasing the mouse. The pointer now turns into a hand over a line to say a click would take it; the click moves the cursor there and **pins it**. Clicking empty space does nothing.
