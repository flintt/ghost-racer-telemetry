...
**Hovering the map only moves the cursor within 18 px of a line.** A generous radius means the shared cursor — and with it every chart, gauge and readout — is stolen whenever the pointer merely crosses the map on its way somewhere else, which reads as the dot chasing the mouse. Click to pin if you want it to stop moving at all.
