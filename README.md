...
- **端口自动焊接**：两条路的端点相距在路宽 + 4 米以内、且**高差不超过 2.5 米**时补一段连接。桥面是独立对象，两侧的路往往差着几米不相接；高差判据保证立交桥不会被粘到下面的路上。
...
**放大后轨迹图会跟着游标走。** 在右侧曲线上移动游标（或把鼠标放到分段表的某一行）时，如果对应的点接近轨迹图边缘，地图会自动平移把它带回来——按最小幅度贴边跟随，滚动时看起来就是地图跟着点滑动；只有目标跳到一屏之外（比如点到另一段分段）才会直接居中。实测 23× 放大下扫完整圈 1200 步，没有一帧把点丢在视野外。手动拖动地图和在地图上直接悬停都不会触发跟随，免得和你的操作打架。
