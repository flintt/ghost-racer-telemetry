...
**轨迹图可以切成行进方向朝上**（右上角 `N` / `▲`），像手机导航那样：当前点固定在偏下的位置，前方的路朝上，地图跟着转。选择记在浏览器里。
...
- **区间表**：`区间 / 最快 / 速率 / 峰值 / 累计领先 / 相对参照圈`，**按速率排序**——排在最上面的就是最该去练的那一段。鼠标划过预览，**点一下这一行就把这段设为选区**（地图加粗高亮、曲线打底色），行尾的 ▶ 直接选中并回放这一段。也就是说"哪儿丢的时间"和"把它放出来看"是同一个动作。
